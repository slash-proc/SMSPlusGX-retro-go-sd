/* Standalone SMSPlus GX dynamic core (this repo). Talks to firmware only
 * through gw_firmware_abi_t — see sdk/src/. gw_core_bridge.h must come after
 * the normal firmware-style headers below so their `extern` declarations of
 * common_emu_state / ACTIVE_FILE / ram_start are parsed first. */
...
{
    /* Cart battery SRAM — AHB heap so RAM_EMU BSS can host YM2413 tables. */
    static uint8 *sram;
    if (sram == NULL) {
        sram = ahb_malloc(0x8000);
        if (sram == NULL) {
            printf("SMS: cart SRAM alloc failed\n");
            return 0;
        }
...
{
    /* Own the DTCM bump for LUT / FM / audio scratch (~104 KiB). */
    dtc_init();
