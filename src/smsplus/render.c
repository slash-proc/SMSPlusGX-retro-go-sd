...
void render_init(void)
{
  ...
  /* SG-1000 / Coleco need TMS LUTs in DTCM (~48 KiB). SMS/GG need the
   * sprite/bg priority lut (~64 KiB). Both do not fit together in ~104 KiB
   * DTCM, so pick based on console (set_config already ran). */
  if (sms.console == CONSOLE_SG1000 || sms.console == CONSOLE_COLECO)
  {
    make_tms_tables();
  }
  else
  {
    ...
    {
      lut = dtc_malloc(0x10000);
      if (lut == NULL)
      {
        printf("render: dtc_malloc(lut) failed\n");
        return;
      }
    }
