...
void make_tms_tables(void)
{
    ...
    if (mc_lookup == NULL)
        mc_lookup = dtc_malloc(sizeof(uint8[16][256][8]));
    if (tms_lookup == NULL)
        tms_lookup = dtc_malloc(sizeof(uint8[16][256][2]));
    if (tms_obj_lut == NULL)
        tms_obj_lut = dtc_malloc(16*256);
    if (txt_lookup == NULL)
        txt_lookup = dtc_malloc(sizeof(uint8[256][2]));
    if (bp_expand == NULL)
        bp_expand = dtc_malloc(sizeof(uint8[256][8]));
